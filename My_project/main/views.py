...
def signup_B(request):
    if request.method == 'GET':
        form = forms.B_signup_form()
        context = {
        'user_signup_form': form
        }
        return render(request, 'B_signup.html', context)

    else:
        form = forms.B_signup_form(request.POST)
        if form.is_valid():
            obj = form.save()
            return HttpResponse("Form Submitted Successfully")
    context = {
...
def login_B(request):
    if request.method == 'GET':
        B_formlogin = forms.B_login_form()
        context = {
            'user_login_form': B_formlogin
        }

        return render(request, 'B_login.html', context)
    else:
        B_formlogin = forms.B_login_form(request.POST)
        email_emp=B_formlogin.data["email_emp"]
        password_emp=B_formlogin.data["password_emp"]
        user = Employee.objects.filter(email_emp=email_emp,password_emp=password_emp)
        print(user)

        if len(user)>0:
            return HttpResponseRedirect('/dashboard_B/')
        else:
            context = {
                'user_login_form': B_formlogin
            }
...
def dashboard_B(request):
    ...
    Sum=User.objects.aggregate(Sum('balance'))
    context = {"Sum":Sum,
               "users": user}
    return render(request, 'B_dashboard.html', context)
